...
public class SA {
	...
	void runSA() {
		...
		do {
			...
			if(noOfLoop==0) {
				noOfIterations++;
				//System.out.println(Math.log(noOfIterations+1));
				temp = 1/Math.log(noOfIterations+1);
				//temp = startingTemp*Math.exp(noOfIterations*Math.log(0.001/startingTemp)/noOfLoop-1);
				stablizer=(int) (stablizer*0.9);
				noOfLoop = stablizer;
			}else {
				noOfLoop--;
			}
			...
			System.out.println(temp+" ---- "+cost+" ---- "+probability+" >= "+random);
		}while(temp>0.0001 && nextStateConflicts != 0 && System.currentTimeMillis()-startTime<=295000);
	...
	int[][] generateNextSolution(){
		...
		while(repeat) {
			...
			outer : for(int i=0;i<noOfRows;i++) {
				if(next[row][i]==1) {
					while(true) {
						int col = new Random().nextInt(noOfRows);
